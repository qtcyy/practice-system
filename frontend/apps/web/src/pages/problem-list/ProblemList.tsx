...
import { Button, Spin, Empty, Card, Tag, Toast } from '@douyinfe/semi-ui-19';
...
const ProblemList = () => {
  ...
  const handleStartPractice = () => {
    ...
    if (firstProblem) {
      navigate(`/app/practice/${setId}?problemId=${firstProblem.id}`);
    } else {
      Toast.warning('题集中没有题目，请先添加题目');
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {!initLoading && problems.length === 0 && (
        <Empty title="暂无题目" description="点击上方按钮添加第一道题目" />
      )}
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 mt-3 text-xs text-gray-400">
                      <span>
                        创建时间:{' '}
                        {new Date(problem.createAt).toLocaleDateString()}
                      </span>
